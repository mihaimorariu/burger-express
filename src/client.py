...
class Client(pygame.sprite.Sprite):
    def __init__(self, table, game):
        ...
        self.image_no_client, self.rect_no_client = LoadImage(
            '../images/Customers/Half Size', 'no_client.png', None)
        self.image, self.rect = self.image_no_client, self.rect_no_client
        self.rect.bottomleft = (
            55 + self.table_number * 180, self.game.restaurant_counter.rect.topleft[1])
    ...
    def update(self):
        if self.active == False:
            # An order has been placed by this client
            if self.game.clients_orders[self.table_number] > 0:
                ...
                self.game.clients_image_ids[self.table_number] = mood * \
                    10 + image_index
                ...
                self.image, self.rect = LoadImage(
                    '../images/Customers/Half Size', file_name, None)
                self.rect.bottomleft = (
                    55 + self.table_number * 180, self.game.restaurant_counter.rect.topleft[1])
                self.active = True
        else:
            # This client left the table
            if self.game.clients_orders[self.table_number] == 0:
                self.image, self.rect = self.image_no_client, self.rect_no_client
