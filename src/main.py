...
class Game:
    ingredients_file_names = ['b_Coca_Cola', 'b_Diet_Coke', 'b_Sprite',
                              'b_Pork2', 'b_Sausage1', 'b_Cheese2', 'b_Fries2', 'b_Bread', 'b_Kroket3']
    sounds_file_names = ['POL-funk-break-short.wav',
                         'POL-80s-pop-short.wav', 'POL-love-line-short.wav']
    ...
    def __init__(self, width, height):
        ...
        self.dish_price = [3, 3, 4, 4, 5, 4, 3, 3, 3,
        ...
        for index in range(0, number_of_ingredients // 3):
            ...
            button_left = IngredientButton(
                index * 3, file_name, position, self)
            counter_left = IngredientCounterWidget(
                index * 3, self, button_left.rect.bottomright)

            file_name = Game.ingredients_file_names[index * 3 + 1]
            position = (80, self.background.get_height() -
                        (index + 1) * 79 - 2)
            button_middle = IngredientButton(
                index * 3 + 1, file_name, position, self)
            counter_middle = IngredientCounterWidget(
                index * 3 + 1, self, button_middle.rect.bottomright)

            file_name = Game.ingredients_file_names[index * 3 + 2]
            position = (159, self.background.get_height() -
                        (index + 1) * 79 - 2)
            button_right = IngredientButton(
                index * 3 + 2, file_name, position, self)
            counter_right = IngredientCounterWidget(
                index * 3 + 2, self, button_right.rect.bottomright)
            ...
            file_name = Game.ingredients_file_names[index * 3]
            position = (self.background.get_width() / 2 - 119,
                        self.background.get_height() / 2 + 39 - index * 79)
            button_left = IngredientRefillButton(
                index * 3, file_name, position, self)
            price_left = IngredientPriceWidget(
                index * 3, self, button_left.rect.bottomright)

            file_name = Game.ingredients_file_names[index * 3 + 1]
            position = (self.background.get_width() / 2 - 40,
                        self.background.get_height() / 2 + 39 - index * 79)
            button_middle = IngredientRefillButton(
                index * 3 + 1, file_name, position, self)
            price_middle = IngredientPriceWidget(
                index * 3 + 1, self, button_middle.rect.bottomright)

            file_name = Game.ingredients_file_names[index * 3 + 2]
            position = (self.background.get_width() / 2 + 39,
                        self.background.get_height() / 2 + 39 - index * 79)
            button_right = IngredientRefillButton(
                index * 3 + 2, file_name, position, self)
            price_right = IngredientPriceWidget(
                index * 3 + 2, self, button_right.rect.bottomright)
        ...
        self.background.blit(self.money_timer_widget.image,
                             self.money_timer_widget.rect)
        ...
        self.other_sprites = pygame.sprite.OrderedUpdates((self.restaurant_counter, self.restaurant_kitchen, self.kitchen_tray, self.cancel_button, self.phone_button_widget, self.question_mark_widget, self.tray_slots, self.money_timer_widget, self.customer_satisfcation_widget,
                                                           self.clients_dish_preference, self.clients_remaining_time, self.ingredients_buttons, self.ingredients_counters, self.refill_buttons, self.refill_prices, self.recipe_book_window, self.welcome_message_window, self.win_lost_widget, self.hand_cursor))
        ...
        self.background_music = pygame.mixer.Sound(
            '../sounds/' + self.sounds_file_names[self.level - 1])
    ...
    def ProcessInputData(self):
        if self.unsatisfied_customers == self.max_number_of_unsatisfied_customers:
            ...
            if self.pause == False:
                ...
                self.background_music = pygame.mixer.Sound(
                    '../sounds/ThePriceIsRight-LoserHorns.wav')
                self.background_music.play()

        if self.pause == False:
            if self.time_until_new_clients == 0:
                # maybe we can let the time be inlfuenced by the level of the game (easy->longer time)
                self.time_until_new_clients = random.randint(1, 5)
                ...
                if number_of_available_places > 0:  # if there are available places, generate a new client
                    ...
                    random_position = random.randint(
                        0, number_of_available_places-1)
                    table_number = available_places[random_position]
                    self.clients_type[table_number] = client_mood
                    # this might need to change later on, so that a new client can order anything
                    self.clients_orders[table_number] = random.randint(
                        (self.level - 1) * 8 + 1, self.level * 8)
                ...
                if self.clients_waiting_time[table_number] == 0:
                    ...
                    if self.clients_orders[table_number] > 0:
                        ...
                        fail_sound = pygame.mixer.Sound(
                            '../sounds/Wheel-of-Fortune-Buzzer.wav')
                        fail_sound.play()
            ...
            if int(time.time() - self.time_start) != self.elapsed:
                ...
                if (self.minutes // self.number_of_minutes_per_level) + 1 > self.level:  # New level
                    self.level = (self.minutes //
                                  self.number_of_minutes_per_level) + 1
                    self.background_music.stop()
                    self.background_music = pygame.mixer.Sound(
                        '../sounds/' + self.sounds_file_names[(self.level - 1) % self.max_levels])
                    self.background_music.play(-1)
