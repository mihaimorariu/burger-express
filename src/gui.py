...
class IngredientCounterWidget(pygame.sprite.Sprite):
    ...
    def update(self):
        self.image = self.font.render(' {0} '.format(
            str(self.game.ingredients_stock[self.index])), True, (255, 255, 255), (0, 0, 0))
...
class IngredientPriceWidget(pygame.sprite.Sprite):
    ...
    def update(self):
        if self.game.show_ingredients_to_order == True:
            self.image = self.font.render(' 5x{0}$ '.format(
                str(self.game.ingredients_price[self.index])), True, (255, 255, 255), (0, 0, 0))
        else:
...
class MoneyTimerWidget(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.image = self.font.render(
            ' Money: 0    Time left: 00:00 ', True, (255, 255, 255), (0, 0, 0))
        self.rect = self.image.get_rect()
        self.rect.topright = (game.background.get_rect(
        ).topright[0] - 30, game.background.get_rect().topright[1])
    ...
    def update(self):
        if self.game.seconds == 0:
            minutes = (self.game.number_of_minutes_per_level *
                       self.game.level) - self.game.minutes
            seconds = 0
        else:
            minutes = (self.game.number_of_minutes_per_level *
                       self.game.level) - self.game.minutes - 1
            seconds = 60 - self.game.seconds

        self.image = self.font.render(' Money: {0}    Time left: {1}:{2} '.format(str(
            self.game.money), str(minutes).zfill(2), str(seconds).zfill(2)), True, (255, 255, 255), (0, 0, 0))
        self.rect = self.image.get_rect()
        self.rect.topright = (self.game.background.get_rect(
        ).topright[0] - 30, self.game.background.get_rect().topright[1])
        self.rect.centery = 30
...
class HandCursor(pygame.sprite.Sprite):
    def __init__(self, hand_track):
        pygame.sprite.Sprite.__init__(self)
        self.hand_normal, self.hand_normal_rect = LoadImage(
            '../images/Cursor', 'hand1.png', (255, 255, 255))
        self.hand_grab, self.hand_grab_rect = LoadImage(
            '../images/Cursor', 'fist1.png', -1)
...
class TraySlot(pygame.sprite.Sprite):
    def __init__(self, index, game):
        ...
        self.image_null, self.rect_null = LoadImage(
            '../images', 'Null.jpg', (0, 0, 0))
        self.image, self.rect = self.image_null, self.rect_null
        self.rect.topleft = (418 + (index - 1) * 85,
                             game.background.get_height() - 162)
    ...
    def update(self):
        if len(self.game.ingredients_queue) >= self.index + 1:
            self.image, self.rect = LoadImage(
                '../images/Ingredients Border', self.game.ingredients_file_names[self.game.ingredients_queue[self.index]] + '.jpg', (0, 0, 0))
            self.rect.topleft = (418 + (self.index - 1) *
                                 85, self.game.background.get_height() - 162)
        else:
            self.image, self.rect = self.image_null, self.rect_null
...
class DishPreferenceWidget(pygame.sprite.Sprite):
    def __init__(self, index, game):
        ...
        self.rect.topleft = (
            85 + index * 180, self.game.restaurant_counter.rect.topleft[1] - 290)
    ...
    def update(self):
        if self.game.clients_orders[self.index] > 0:
            self.image, self.rect = LoadImage(
                '../images/Dishes', str(self.game.clients_orders[self.index]) + '.png', (0, 0, 0))
            self.rect.topleft = (
                85 + self.index * 180, self.game.restaurant_counter.rect.topleft[1] - 290)
        elif self.game.clients_orders[self.index] == -1:
            self.image, self.rect = LoadImage(
                '../images', 'OKSign.png', (0, 0, 0))
            self.rect.topleft = (
                85 + self.index * 180, self.game.restaurant_counter.rect.topleft[1] - 290)
        elif self.game.clients_orders[self.index] == -2:
            self.image, self.rect = LoadImage(
                '../images', 'WrongSign.png', (0, 0, 0))
            self.rect.topleft = (
                85 + self.index * 180, self.game.restaurant_counter.rect.topleft[1] - 290)
        else:
            self.image, self.rect = LoadImage(
...
class DishPreferenceCounter(pygame.sprite.Sprite):
    def __init__(self, index, game):
        ...
        self.image, self.rect = LoadImage('../images', 'Null.jpg', (0, 0, 0))
        self.rect.topleft = (
            85 + index * 180, game.restaurant_counter.rect.topleft[1] - 290)
    ...
    def update(self):
        if self.game.clients_waiting_time[self.index] != -1:
            self.image = self.font.render(' {0} '.format(str(
                self.game.clients_waiting_time[self.index])), True, (255, 255, 255), (0, 0, 0))
...
class CancelButton(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.game = game
        self.image, self.rect = LoadImage(
            '../images', 'cancel.png', (255, 255, 255))
        self.rect.topleft = (
            self.game.kitchen_tray.rect.topright[0] + 50, self.game.kitchen_tray.rect.topright[1] + 80)
...
class PhoneButtonWidget(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.rect.topright = (self.game.background.get_width(
        ) - 150, self.game.kitchen_tray.rect.topright[1] + 80)
        self.mouse_over = False
...
class CustomerSatisfactionWidget(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.font = pygame.font.Font(None, 36)
        self.game = game
        self.image = self.font.render('  Level: 1   Unsatisfied: 0/{0}  '.format(
            self.game.max_number_of_unsatisfied_customers), True, (255, 255, 255), (0, 0, 0))
    ...
    def update(self):
        ...
        self.image = self.font.render('  Level: {0}   Unsatisfied: {1}/{2}  '.format(str(level), str(
            self.game.unsatisfied_customers), str(self.game.max_number_of_unsatisfied_customers)), True, (255, 255, 255), (0, 0, 0))
...
class WinLostWidget(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.game = game
        self.image, self.rect = LoadImage('../images', 'Null.jpg', (0, 0, 0))
        self.rect.topleft = (self.game.width / 2 - self.rect.width / 2,
                             self.game.height / 2 - self.rect.height / 2)
...
class QuestionMarkWidget(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.game = game
        self.image, self.rect = LoadImage(
            '../images', 'questionmark.png', (0, 0, 0))
        self.rect.topright = (self.game.background.get_width(
        ) - 30, self.game.kitchen_tray.rect.topright[1] + 80)
        self.mouse_over = False
...
class WelcomeMessageWindow(pygame.sprite.Sprite):
    def __init__(self, game):
        ...
        self.image_welcome, self.rect_welcome = LoadImage(
            '../images', 'WelcomeMessage2.png', None)
        self.image_null, self.rect_null = LoadImage(
            '../images', 'Null.jpg', (0, 0, 0))
