...
#        HandTracking.ycc_max = cv.Scalar(255, 210, 130)
    ...
    def ComputeHandPosition(self, skin_image, hand_contour, hull4defects):
        ...
        if number_of_white_pixels < HandTracking.hand_area_threshold:
            ...
            new_cursor_pos = (sum_of_white_pixels[1] / number_of_white_pixels, sum_of_white_pixels[0] / number_of_white_pixels)
            # Scale the cursor position to the screen resolution.
            image_size = cv.GetSize(skin_image)
            new_cursor_pos = ((2000/image_size[0])*new_cursor_pos[0],(1500/image_size[1])*new_cursor_pos[1])
        
        # Determine hull defects for the convex hull of the detected hand.
        if hand_contour:
            defects = cv.ConvexityDefects(hand_contour, hull4defects, cv.CreateMemStorage())
    ...
    def DetectHandSkin(self, query_image):
        ...
        contours = cv.FindContours(skin, cv.CreateMemStorage(), cv.CV_RETR_LIST, cv.CV_CHAIN_APPROX_SIMPLE)
        hand_contour = None
        max_area = 0
        ...
        if hand_contour:
            cv.Zero(skin)
            hand_contour = cv.ApproxPoly(hand_contour, cv.CreateMemStorage(), cv.CV_POLY_APPROX_DP, 3, 1)
            hull = cv.ConvexHull2(hand_contour, cv.CreateMemStorage(), cv.CV_CLOCKWISE, 1)
            cv.DrawContours(skin, hull, cv.RGB(255, 255, 255), cv.RGB(255, 255, 255), 1, -1)
            hull4defects = cv.ConvexHull2(hand_contour, cv.CreateMemStorage(), cv.CV_CLOCKWISE, 0)
            #cv.DrawContours(skin, hull, cv.RGB(255, 0, 0), cv.RGB(255, 255, 255), 1, 2)

            cv.DrawContours(query_image, hull, cv.RGB(255, 255, 255), cv.RGB(255, 255, 255), 1, 2)
            cv.DrawContours(query_image, hand_contour, cv.RGB(0, 0, 255), cv.RGB(0, 0, 255), 1, 2)
        else:
            cv.Zero(skin)
            hand_contour = False
            hull4defects = False
    ...
    def GetHandPosition(self):
        ...
        hand_skin, hand_contour, hull4defects = self.DetectHandSkin(current_frame)
